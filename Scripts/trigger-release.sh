...
PROJECT_SLUG="gh/contentful/rich-text-renderer.swift"

: "${CIRCLE_TOKEN:?Set CIRCLE_TOKEN to a CircleCI personal API token (https://app.circleci.com/settings/user/tokens)}"

git fetch --quiet origin master
VERSION="$(git show origin/master:ContentfulRichTextRenderer.podspec \
  | sed -n -E 's/^[[:space:]]*spec\.version[[:space:]]*=[[:space:]]*"([^"]*)".*/\1/p')"
...
echo "About to release rich-text-renderer.swift $VERSION from origin/master ($COMMIT)."
if [[ "${1:-}" != "--yes" ]]; then
